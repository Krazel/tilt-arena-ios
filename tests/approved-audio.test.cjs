...
test('only selected files ship, with verified hashes, credits and half-second charges',()=>{
 ...
 assert.deepEqual([...proposals].sort(),['music-play-A','music-play-B','music-play-C','music-menu-A','click-A','pickup-A','kill-A','vortex-A','lightning-A','burn-A','boomerang-A','shatter-A','shatter-B','bounce-B'].sort());
...
test('real power events route only approved cues; silent powers retain just pickup',async()=>{
 const {soundCues}=await modulePromise;
 for(const power of ['nuke','wave','missiles','frost','bubble','spikes']){const game=new ClassicGame(7,{spawning:false});game.activate(power);assert.deepEqual(soundCues(game.snapshot().events),['pickup']);}
 ...
 assert.deepEqual(soundCues([{kind:'death'},{kind:'combo'},{kind:'warning'},{kind:'freeze'},{kind:'lightning'}]),[]);
});
