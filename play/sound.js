...
export function soundCues(events, boomerangCharging=false) {
  ...
  for(const e of events){
    if(e.kind==='pickup'){cues.push('pickup');if(['burn','boomerang'].includes(e.power))cues.push(e.power+'-charge');}
...
export class GameSound {
  constructor(catalog, makeAudio=file=>new Audio('/'+file), now=()=>performance.now()/1000){
    ...
    this.track=-1;this.current=null;this.last={};this.paused=new Set();this.shatter=0;this.lastFrame=null;this.vortex=false;
    for(const [name,a] of Object.entries(catalog.assets)){const p=makeAudio(a.file);p.preload='auto';p.volume=a.volume;p.loop=name===catalog.menu||name==='vortex';this.players[name]=p;
  ...
  music(){const wanted=this.mode==='menu'?this.catalog.menu:this.mode==='game'?this.current:null;for(const [n,p] of Object.entries(this.players))if(n.startsWith('music')){if(this.audible&&n===wanted)this.playPlayer(n);else p.pause();}}
  ...
  setSuspended(value){this.suspended=value;if(value)Object.values(this.players).forEach(p=>p.pause());else this.music();}
  startRun(){this.clearEffects();this.last={};this.lastFrame=null;this.vortex=false;this.advanceTrack();this.players[this.current].currentTime=0;this.mode='game';this.music();}
  setMode(next){const prev=this.mode;this.mode=next;if(next!=='game'){if(next==='paused'){for(const [n,p]of Object.entries(this.players))if(!n.startsWith('music')&&n!=='ui'&&!p.paused){this.paused.add(n);p.pause();}}else{this.clearEffects();this.vortex=false;}}else if(prev==='paused'&&this.audible){for(const n of this.paused)this.playPlayer(n);this.paused.clear();}this.music();this.refreshVortex();}
  refreshVortex(){const p=this.players.vortex;if(this.audible&&this.mode==='game'&&this.vortex){if(p.paused)this.playPlayer('vortex');}else{p.pause();if(!this.vortex)p.currentTime=0;}}
  ...
  consume(frame){if(this.mode!=='game'||frame.state!=='running'||frame.time===this.lastFrame)return;this.lastFrame=frame.time;this.vortex=frame.fields.some(f=>f.kind==='vortex'&&f.remaining>0);this.refreshVortex();for(const cue of soundCues(frame.events,frame.player.boomerangCharging))this.play(cue);}
}
